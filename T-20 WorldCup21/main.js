...
// npm install pdf-lib
// node main.js --excel=Worldcup.csv --dataDir=worldcup --source=https://www.espncricinfo.com/series/icc-men-s-t20-world-cup-2021-22-1267897/match-results
...
responseKaPromise.then(function (response) {
    ...
    for (let i = 0; i < matchScoreDivs.length; i++) {
        ...
        let teamParas = matchScoreDivs[i].querySelectorAll("div.name-detail > p.name"); // team name
        match.t1 = teamParas[0].textContent;
        match.t2 = teamParas[1].textContent;

        let scoreSpans = matchScoreDivs[i].querySelectorAll("div.score-detail > span.score"); //score
        ...
        let resultSpan = matchScoreDivs[i].querySelector("div.status-text > span"); // result
        match.result = resultSpan.textContent;

        matches.push(match); // adding the details in matches array
    }
...
function prepareFoldersAndPdfs(teams, dataDir) {
    if (fs.existsSync(dataDir) == true) {   // checking if dir is present or not if present delete it
        fs.rmdirSync(dataDir, {
            recursive: true
        });
    }

    fs.mkdirSync(dataDir); // create new

    for (let i = 0; i < teams.length; i++) {
        let teamFolderName = path.join(dataDir, teams[i].name); // adding team names
        fs.mkdirSync(teamFolderName);

        for (let j = 0; j < teams[i].matches.length; j++) {
            let match = teams[i].matches[j];
            createMatchScorecardPdf(teamFolderName, teams[i].name, match); // pdf
        }
    }
}
...
function createMatchScorecardPdf(teamFolderName, homeTeam, match) {
    ...
    pdfdocKaPromise.then(function (pdfdoc) {
        ...
        page.drawText(homeTeam, { // putting the content in right place
            x: 250,
            y: 420,
            size: 14
        });
        ...
        changedBytesKaPromise.then(function (changedBytes) {
            if (fs.existsSync(matchFileName + ".pdf") == true) {
                fs.writeFileSync(matchFileName + "1.pdf", changedBytes); // if already present pdf with same name
            } else {
...
function prepareExcel(teams, excelFileName) {  // creating the excel file
    let wb = new excel4node.Workbook();

    for (let i = 0; i < teams.length; i++) { 
        let tsheet = wb.addWorksheet(teams[i].name);   // adding sheets

        tsheet.cell(1, 1).string("Vs");   //rows
        tsheet.cell(1, 2).string("Self Score");
        tsheet.cell(1, 3).string("Opp Score");
        tsheet.cell(1, 4).string("Result");
        for (let j = 0; j < teams[i].matches.length; j++) {
            tsheet.cell(2 + j, 1).string(teams[i].matches[j].vs);  // filling rows
